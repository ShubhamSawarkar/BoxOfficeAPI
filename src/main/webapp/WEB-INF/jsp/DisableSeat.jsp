...
<title>Disable Seat For A Show</title>
...
<%@ page import = "java.util.List, com.restapi.model.Seat, com.restapi.model.Show, java.util.Collections" %>
	<form id = "chooser" action = "/DisableSeatChange">
	...
	<div id = "hall">
		<%
			...
			Object currentShow = request.getAttribute("show");

			int currentRow = 1;
		
			if((seatsObj != null) && (statusObj != null) &&(currentShow != null))
			{
				@SuppressWarnings("unchecked")
				List<Seat> seats = (List<Seat>) seatsObj;
				@SuppressWarnings("unchecked")
				List<Boolean> status = (List<Boolean>) statusObj;
				Show show = (Show) currentShow;
				
				out.println("<form action = \"reserveSeats\" method = \"post\">");
				...
				for(Seat seat: seats)
				{
					...
					out.println("<input type = \"checkbox\" value = \"" + seat.getSeatID() + " " + show.getID() + "\" class = " + ((status.get(seats.indexOf(seat)) == true) ? "available" : "reserved") +" disabled = " + ((status.get(seats.indexOf(seat)) == true) ? false : true) +" name = \"checked\">");
				...
				out.println("<input type = \"submit\" class = \"showselectorbtn\" value = \"Reserve\"");
			}
		%>
	</div>
